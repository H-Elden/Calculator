...
void Cal() {
    ...
    if (r == '!') {        // 阶乘只需要前面有一个数字，单独处理
        if (b == 0)
            h.push(1);
        else if (b > 0 && (unsigned long long)b == b) {
            double result = 1;
            while (b) {
                result *= b;
                b      -= 1;
...
void outputResult() {
    switch (errorFlag) {
        ...
        case RESULT_NAN:         puts("ERROR: The result is NaN (Not a Number)."); break;
        case RESULT_INF:         puts("ERROR: The result is too big."); break;
        case UNKNOWN_ERROR:
